...
impl Bank {
    ...
    #[request_handler]
    async fn handle_new_account(&mut self, msg: NewAccount, _ctx: &Context<Self>) -> MsgResult {
    ...
    #[request_handler]
    async fn handle_deposit(&mut self, msg: Deposit, _ctx: &Context<Self>) -> MsgResult {
        match self.accounts.get(&msg.who) {
            Some(current) => {
    ...
    #[request_handler]
    async fn handle_stop(&mut self, _msg: Stop, ctx: &Context<Self>) -> MsgResult {
        ctx.stop();
