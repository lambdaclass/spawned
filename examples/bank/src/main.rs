...
fn main() {
    rt::run(async {
        ...
        let result = Bank::withdraw(&name_server, "main".to_string(), 15).await;
        ...
        // Deposit
        let result = Bank::deposit(&name_server, "Joe".to_string(), 30).await;
        tracing::info!("Deposit result {result:?}");
        assert_eq!(
            result,
            Ok(BankOutMessage::Balance {
        ...
        // Withdrawal
        let result = Bank::withdraw(&name_server, "Joe".to_string(), 15).await;
        tracing::info!("Withdraw result {result:?}");
        assert_eq!(
            result,
            Ok(BankOutMessage::WidrawOk {
        ...
        let result = Bank::withdraw(&name_server, "Joe".to_string(), 45).await;
        tracing::info!("Withdraw result {result:?}");
        assert_eq!(
            result,
        ...
        // Full withdrawal
        let result = Bank::withdraw(&name_server, "Joe".to_string(), 25).await;
        tracing::info!("Withdraw result {result:?}");
        assert_eq!(
            result,
            Ok(BankOutMessage::WidrawOk {
        ...
        // Stopping the bank
        let result = Bank::stop(&name_server).await;
