...
fn main() {
    rt::run(|| {
        ...
        let result = bank.withdraw("main".into(), 15).unwrap();
        ...
        // Deposit
        let result = bank.deposit(joe.clone(), 30).unwrap();
        tracing::info!("Deposit result {result:?}");
        assert_eq!(
            result,
            Ok(BankOutMessage::Balance {
        ...
        // Withdrawal
        let result = bank.withdraw(joe.clone(), 15).unwrap();
        tracing::info!("Withdraw result {result:?}");
        assert_eq!(
            result,
            Ok(BankOutMessage::WithdrawOk {
        ...
        let result = bank.withdraw(joe.clone(), 45).unwrap();
        tracing::info!("Withdraw result {result:?}");
        assert_eq!(
            result,
        ...
        // Full withdrawal
        let result = bank.withdraw(joe.clone(), 25).unwrap();
        tracing::info!("Withdraw result {result:?}");
        assert_eq!(
            result,
            Ok(BankOutMessage::WithdrawOk {
